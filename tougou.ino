...
// GPSの設定
TinyGPSPlus gps;
const double Kp_gps = 0.65; // P制御の比例ゲイン
...
String FILE_NAME = "0720etoe"; // ファイル名
String file_name;
...
void AzimuthDistance() {
  ...
  double turnpower = azimuth - eulerdata[2];  
  azidata[0] = turnpower;
...
void P_GPS_Moter() { 
  ...
  while(1){
    ...
    if(azidata[1] < 5){
      ...
      int PID_left = constrain(Kp_gps * azidata[0] + 207, 0, 255);
      int PID_right = constrain(- Kp_gps * azidata[0] + 207, 0, 255);
