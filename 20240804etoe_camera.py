...
threshold_red = (49, 76, 7, 127, -128, 74)
...
while True:
    ...
    blobs_red = img.find_blobs([threshold_red], x_stride=2, y_stride=2, pixels_threshold=100, merge=False, margin=20)
    if blobs_red:
        max_blob_red = max(blobs_red, key=lambda b: b.area())  # 面積が最大の領域を取得
        max_per_red = max_blob_red.area() / IMAGE_TOTAL_PIXELS*100
        ...
        if max_per_red > 0.5:
            ...
            sendstr = "R-1,-1,0.0O0,0,0.0Y0,0,0.0"
            uart.write(sendstr + "\n")  # 文字列を一括で送信
            print("0,0")
    else:
        sendstr = "R-1,-1,0.0O0,0,0.0Y0,0,0.0"
